...
def ping_via_ssh_port(host, port) -> bool:
...
def progress(sent, to_sent) -> None:
    ...
    sys.stdout.write("progress: %.2f%% \r" % (float(totalsent/total_files_size)*100))
    # sys.stdout.flush()

def args() -> dict[str]:
    args_dict = {}
    parser = argparse.ArgumentParser()
    parser.add_argument("-e", "--edit_conf",
    help="opens cactos.conf.json to edit manualy",action="store_true")
    args = parser.parse_args()
    
    parser = argparse.ArgumentParser()
    parser.add_argument("-s", "--use_saved_conf",
    help="use defualt seved in : cactus.conf.json, (need to be in cactus directory).",action="store_true")
    args = parser.parse_args()
    
    parser = argparse.ArgumentParser()
    parser.add_argument("-i", "--set_ip",
    help="set new ip and keep rest of paramaters in cactus.conf.json file",action="store_true")
    args = parser.parse_args()
...
def manual_paramters(OS):
    ...
    save_to_next_time = input("do you wish to save configuration to next time? [y/N]: ").upper()
...
if __name__ == "__main__":
    ...
    if args_dict["use_defualts"]():
        if "cactus.conf.json" in os.listdir(os.getcwd()):
            ...
            answer = input("Cant find configuration files.. do you want to enter configuration manualy? [y/N]").upper()
            if answer == "Y":
                (
                    HOSTNAME,
                    PORT,
    ...
    if ping_via_ssh_port(HOSTNAME, PORT):    
        ...
        server_dict, sftp_client = get_remote_hashes(HOSTNAME, USERNAME, KEY, PORT, SERVER_PATH)
        total_files_size = sum([os.stat(client_dict[i]).st_size for i in client_dict if i not in server_dict])
        totalsent, allready_sent = 0, 0
        print("copying files...")
        files_count = 0
        for i in client_dict:
            if i not in server_dict:
                sftp_client.put(client_dict[i], SERVER_PATH+(client_dict[i].lstrip(CLIENT_PATH)), callback=progress)
                files_count += 1
